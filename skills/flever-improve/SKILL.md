...
- `<gate-status>IMPROVE: <files touched></gate-status>` — diff staged for the user.
- `<gate-status>SKIP: <reason></gate-status>` — the trace yielded no reusable rule worth a meta-process change.

The diff sits in the working tree; the user decides how to integrate it.
