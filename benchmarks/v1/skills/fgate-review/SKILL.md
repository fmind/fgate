...
End with the chosen primary action on its own line, then a single-line `Other options:` footer naming the alternatives.

Example:

```text
Ship it — `feat(auth): add password sign-in` ready; 6/6 verifiers pass. Diff staged for the user.
