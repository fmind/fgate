...
Six gates (`init` → `prompt` → `plan` → `implement` → `review` → `improve`) run as Agent Skills across Claude Code, Gemini CLI, and GitHub Copilot from a single source.
...
- Skill names use the `fgate-` prefix; heavy reference material lives in sibling files, not the skill body.
...
- Every gate ends with exactly one `<gate-status>...</gate-status>` tag on its own line. Hosts grep for it to chain gates.

## Workflow

- State is files: presence of `prompt.md` → `plan.md` → `result.md` under `.agents/gates/<id>/` advances the workflow.
- Two artifacts per gate: `human/<gate>.md` (30-second skim) and `agent/<gate>.md` (reproducible detail). Once written, treat them as read-only.
